...
function SecretGardenReveal({get}){return <div className="bt-grove-opening" role="status"><div className="bt-grove-opening__world" aria-hidden="true"><span/><span/><span/><i>⋈</i><i>✦</i></div><p className="br-kicker">{get('groveOpeningLabel')}</p><h2>{get('groveOpening')}</h2><p>{get('groveOpeningHint')}</p></div>}
...
export default function PlayBowracle({onBackToPlay,onStoreHandoff}){
 ...
 useEffect(()=>{if(page!=='intro'&&page!=='secret'){heading.current?.focus({preventScroll:true});heading.current?.scrollIntoView({block:'start',behavior:'auto'});}},[page]);
 ...
 function openSecret(){setName('');setResult(null);setPage('secret');}
 ...
 const secretRitual=page==='secret'||(page==='processing'&&pending.current?.mode==='house')||(page==='result'&&result?.mode==='house');
 ...
 return <main className={'br-shell bt-shell br-shell--'+page+(secretRitual?' bt-secret-ritual':'')+(paused?' br-motion-paused':'')+(reduced?' bt-reduced-motion':'')}>
  ...
  <section className="bt-page">{page!=='secret'&&<header className="bt-page-heading"><p className="br-kicker">{get('table')}</p><h1 ref={heading} tabIndex={-1}>{title}</h1>{['major','minor','formation'].includes(page)&&<p>{get(page+'Hint')}</p>}</header>}
   ...
   {page==='processing'&&pending.current?.mode==='house'?<SecretGardenReveal get={get}/>:((page==='formation'||page==='processing')&&<><div className={'bt-formation'+(page==='processing'?' is-revealing':'')} aria-label={get('revealCards')}>{cards.map((item,i)=><div className="bt-formation-card" style={{'--i':i}} key={item.card.id}><div className="bt-card-turn"><div className="bt-card-back"><TarotSigil/></div><div className="bt-card-front" aria-hidden={page!=='processing'}><TarotFace card={item.card} kind={item.kind} t={t}/></div></div></div>)}</div>{page==='formation'&&<div className="bt-formation-controls"><NameField value={name} onChange={setName} t={t}/><div className="br-actions"><button type="button" className="br-link" onClick={()=>setPage('minor')}>← {get('back')}</button><button type="button" className="br-button" onClick={()=>reveal(createTarotReading({majorArcanaIds:major,minorArcanaIds:minor}))}>{get('reveal')} ✧</button></div></div>}{page==='processing'&&<p className="bt-processing-note" role="status">{get('processing')}</p>}</>)}
   {page==='secret'&&<><SecretCodeForm t={t} name={name} onName={setName} onReveal={reveal}/><button type="button" className="br-link bt-grove-back" onClick={()=>setPage('intro')}>← {get('back')}</button></>}
