...
export default function PlayBowracle({onBackToPlay,onStoreHandoff}){
 ...
 return <main className={'br-shell bt-shell br-shell--'+page+(paused?' br-motion-paused':'')+(reduced?' bt-reduced-motion':'')}>
  ...
  <section className="bt-page"><header className="bt-page-heading"><p className="br-kicker">{get('table')}</p><h1 ref={heading} tabIndex={-1}>{title}</h1>{['major','minor','formation'].includes(page)&&<p>{get(page+'Hint')}</p>}</header>
   ...
   {(page==='formation'||page==='processing')&&<><div className={'bt-formation'+(page==='processing'?' is-revealing':'')} aria-label={get('revealCards')}>{cards.map((item,i)=><div className="bt-formation-card" style={{'--i':i}} key={item.card.id}><div className="bt-card-turn"><div className="bt-card-back"><TarotSigil/></div><div className="bt-card-front" aria-hidden={page!=='processing'}><TarotFace card={item.card} kind={item.kind} t={t}/></div></div></div>)}</div>{page==='formation'&&<div className="bt-formation-controls"><NameField value={name} onChange={setName} t={t}/><div className="br-actions"><button type="button" className="br-link" onClick={()=>setPage('minor')}>← {get('back')}</button><button type="button" className="br-button" onClick={()=>reveal(createTarotReading({majorArcanaIds:major,minorArcanaIds:minor}))}>{get('reveal')} ✧</button></div></div>}{page==='processing'&&<p className="bt-processing-note" role="status">{get('processing')}</p>}</>}
