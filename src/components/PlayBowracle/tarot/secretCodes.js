...
export async function resolveBowracleSecretCode(value,{demo=false,lookup}={}){
 ...
 try{
  ...
  if(!BOWRACLE_HOUSES.some(h=>h.id===record.house)||typeof record.readingSeed!=='string'||!record.readingSeed)return {valid:false,reason:'invalid'};
  ...
  return {valid:true,code,house:record.house,readingSeed:record.readingSeed,edition:typeof record.edition==='string'?record.edition.slice(0,60):null,bowConfig,status:record.status,demo:!lookup};
