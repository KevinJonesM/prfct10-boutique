...
import { createPowerPackBlobs, createPowerPackSvgs, downloadPowerPack, downloadPowerStory } from "./powerCardCanvas";
import { getApparatusAsset } from "./apparatusAssets";
import { normalizePowerQuote } from "./data/powerQuotes";

export function getResultCopy(result, t, locale = result.locale) {
  const isSpanish = locale === "es";
  const normalizedQuote = normalizePowerQuote({ en: result.quoteEn, es: result.quoteEs });
  const selectedQuote = isSpanish ? normalizedQuote.es : normalizedQuote.en;
  return {
    today: t("play.powerCheck.result.today"),
    identity: isSpanish ? (result.todayYoureEs || "MI PODER") : (result.todayYoureEn || "MY POWER"),
    identityLine: t(`play.identities.${result.apparatusId || result.apparatus}.line`),
    apparatus: t(`play.powerCheck.options.apparatus.${result.apparatusId || result.apparatus}`),
    energy: t(`play.energyMessages.${result.currentEnergyId || result.currentEnergy}`),
    mantra: selectedQuote,
    ritualLabel: t("play.powerCheck.result.ritualLabel"),
    ritual: t(`play.rituals.${result.intentionId || result.intention}`),
...
export default function PowerResult({ result, onTryAgain, onBackToPlay, t, locale }) {
  ...
  const copy = useMemo(() => getResultCopy(result, t, locale), [result, t, locale]);
  const pack = useMemo(() => createPowerPack({ result, copy, locale, t }), [result, copy, locale, t]);
  const colors = [result.primaryColorId, result.secondaryColorId].map((id) => ALL_POWER_COLORS.find((color) => color.id === id)).filter(Boolean);
  const storySvgs = useMemo(() => createPowerPackSvgs({ pack }), [pack]);
  ...
  const exportStories = async (share) => {
    ...
    try {
      const blobs = await createPowerPackBlobs({ pack, svgs: storySvgs });
  ...
  return <section className="power-result" aria-labelledby="power-result-title" aria-live="polite">
    <div className="power-result__brand"><span>PRFCT10</span><b>PLAY</b><i>POWER REVEAL</i></div>
    <div className="power-result__layout"><div className="power-result__card">
      ...
      <div className="power-result__profile"><div className="power-result__apparatus"><span>{t("play.powerCheck.result.apparatusLabel")}</span><div><img src={getApparatusAsset(result.apparatusId || result.apparatus)} alt="" aria-hidden="true" /><strong>{copy.apparatus}</strong></div></div><div><span>{t("play.powerCheck.result.colorsLabel")}</span><div className="power-result__colors">{colors.map((color) => <span key={color.id} style={{ "--result-color": color.value }}><i aria-hidden="true" /> {t(`play.colors.${color.id}`)}</span>)}</div></div></div>
      ...
      <div className="power-pack__viewport" aria-label={t("play.powerCheck.result.previewLabel")} dangerouslySetInnerHTML={{ __html: storySvgs[activeStory] }} />
