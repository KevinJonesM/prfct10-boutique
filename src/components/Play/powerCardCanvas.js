...
function svgShell(body, background, foreground, defs = "") { return `<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1920" viewBox="0 0 1080 1920" data-template="${POWER_CHECK_BASE_ASSET}"><defs>${defs}</defs><rect width="1080" height="1920" fill="${background}"/>${body}</svg>`; }
function logo() { return assetImage(POWER_PLAY_ASSETS.mark, 62, 112, 190, 176, 1); }
...
function storyTwo(s) {
  ...
  return svgShell(`<rect width="1080" height="1920" fill="#FFF9F2"/><rect y="0" width="1080" height="92" fill="url(#check2)"/>${logo()}<g transform="translate(350 158)"><rect width="640" height="72" fill="#17131F"/><text x="320" y="48" text-anchor="middle" fill="${accent}" font-family="Nunito Sans,Arial,sans-serif" font-size="27" font-weight="900" letter-spacing="5">${esc(s.label).toUpperCase()} / 02</text></g>${centeredTextLines(s.mixLabel, 540, 410, 900, 88, "#17131F", 2, 900)}<rect x="90" y="550" width="900" height="86" fill="#17131F"/><text x="540" y="610" text-anchor="middle" fill="#fff" font-family="Nunito Sans,Arial,sans-serif" font-size="42" font-weight="900" letter-spacing="3">${esc(s.mix).toUpperCase()}</text>${panels}<g transform="translate(90 1535)" filter="url(#shadow2)"><rect width="900" height="190" fill="#fff" stroke="#17131F" stroke-width="5"/>${centeredTextLines(`“${s.quote}”`, 450, 80, 770, 48, "#17131F", 3, 900)}</g><text x="540" y="1810" text-anchor="middle" fill="#17131F" font-family="Nunito Sans,Arial,sans-serif" font-size="21" font-weight="900" letter-spacing="4">TODAY YOU’RE… ${esc(s.todayYoure).toUpperCase()}</text>`, "#FFF9F2", "#17131F", defs);
}
...
async function svgToBlob(svg) {
  const source = new Blob([svg], { type: "image/svg+xml;charset=utf-8" }); const url = URL.createObjectURL(source); const image = new Image(); image.src = url; await image.decode(); URL.revokeObjectURL(url);
  const canvas = document.createElement("canvas"); canvas.width = POWER_CARD_DIMENSIONS.width; canvas.height = POWER_CARD_DIMENSIONS.height; canvas.getContext("2d").drawImage(image, 0, 0, 1080, 1920); return new Promise((resolve, reject) => canvas.toBlob((blob) => blob ? resolve(blob) : reject(new Error("Unable to create PNG")), "image/png", 1));
}
