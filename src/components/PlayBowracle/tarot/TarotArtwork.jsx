...
export function TarotFace({card,kind,t}){
 ...
 if(kind==='major'&&illustratedMajorArcana.has(card.id))return <span className="bt-card-face bt-card-face--illustrated"><img src={'/assets/bowracle/major-arcana/'+card.id+'.webp'} alt={t(key+'.name')+'. '+t(key+'.microcopy')} loading="lazy"/></span>;
