...
export function createTarotSvg(result,t,format='story',{kind='reading',firstName='',prefix='tarot-card'}={}){
 ...
 const text=(value,x,y,size,extra='')=>`<text x="${x}" y="${y}" font-size="${size}" font-family="Arial,sans-serif" fill="#f6edfc" ${extra}>${esc(value)}</text>`;
 ...
 let output=`<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="${height}" viewBox="0 0 1080 ${height}"><defs><linearGradient id="${prefix}-bg" x2="1" y2="1"><stop stop-color="#271737"/><stop offset=".6" stop-color="#170e26"/><stop offset="1" stop-color="#37385c"/></linearGradient></defs><rect width="1080" height="${height}" fill="#180e26"/><rect x="25" y="25" width="1030" height="${height-50}" rx="75" fill="url(#${prefix}-bg)" stroke="${accent}" stroke-width="3"/><rect x="43" y="43" width="994" height="${height-86}" rx="62" fill="none" stroke="#ebcaff45"/>`;
