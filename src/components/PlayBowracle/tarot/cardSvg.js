...
export function createTarotSvg(result,t,format='story',{kind='reading',firstName='',prefix='tarot-card'}={}){
 ...
 const garden=houseCard&&result.mode==='house',ink=houseCard?'#2b1830':'#f6edfc';
 const backgroundArt=houseCard?'/assets/bowracle/mystic-pop/house-collectible-v1.webp':'/assets/bowracle/mystic-pop/reading-collectible-v1.webp';
 ...
 let output=`<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="${height}" viewBox="0 0 1080 ${height}"><defs><linearGradient id="${prefix}-bg" x2="1" y2="1"><stop stop-color="${houseCard?'#fffaf0aa':'#27173766'}"/><stop offset=".6" stop-color="${houseCard?'#f2e3eaaa':'#170e2688'}"/><stop offset="1" stop-color="${houseCard?'#dceae3aa':'#37385c66'}"/></linearGradient></defs><rect width="1080" height="${height}" fill="${houseCard?'#fbf5e9':'#180e26'}"/><image href="${backgroundArt}" x="0" y="0" width="1080" height="${height}" preserveAspectRatio="xMidYMid slice"/><rect x="25" y="25" width="1030" height="${height-50}" rx="75" fill="url(#${prefix}-bg)" stroke="${houseCard?'#fff7fc':accent}" stroke-width="4"/><rect x="43" y="43" width="994" height="${height-86}" rx="62" fill="none" stroke="${houseCard?'#a2709488':'#ebcaff88'}"/>${botanical}<rect x="208" y="57" width="664" height="108" rx="42" fill="${houseCard?'#fffaf0cc':'#10071ddd'}" stroke="${houseCard?'#d9a7c9':'#f2c8ff66'}"/>`;
